...
class SummonMonitor:
    def __init__(self):
        ...
        self.update_interval = 2000  # Increased update interval to reduce flickering
    ...
    def find_summons(self):
        ...
        try:
            ...
            filter.Enabled = True
            filter.RangeMax = 30  # Increased range to 30 tiles
    ...
    def create_static_gump(self):
        """Create only the static background gump (no health bars)"""
        width = 200
        height = max(25, len(self.summons) * 25)
        gd = Gumps.CreateGump(movable=True)
        Gumps.AddPage(gd, 0)
        Gumps.AddBackground(gd, 0, 0, width, height, 30546)
        Gumps.AddAlphaRegion(gd, 0, 0, width, height)
        Gumps.SendGump(self.gump_id, Player.Serial, 700, 700, gd.gumpDefinition, gd.gumpStrings)

    def create_dynamic_gump(self):
        """Create only the dynamic (health bar) layer, to be drawn over the static background, using MOBA-style bar art and hues"""
        MOBA_BAR_ART = 5210  # 12x16 pixel bar segment
        MOBA_SEGMENT_WIDTH = 12
        MOBA_SEGMENT_HEIGHT = 16
        MOBA_BAR_WIDTH = 60
        y_offset = 2
        gd = Gumps.CreateGump(movable=True)
        for serial, summon in self.summons.items():
            ...
            if summon.get('out_of_range', False):
                ...
                if health_percent >= 0.7:
                    ...
                    color = 38   # Bright red
            num_segments = MOBA_BAR_WIDTH // MOBA_SEGMENT_WIDTH
            filled_segments = int((current_hits / max_hits) * num_segments) if max_hits > 0 else 0
            bar_x = 135
            bar_y = y_offset + 2
            for i in range(num_segments):
                Gumps.AddImage(gd, bar_x + i * MOBA_SEGMENT_WIDTH, bar_y, MOBA_BAR_ART, 2999)
    ...
    def create_gump(self):
        """Create a compact health bar gump with static and dynamic separation"""
        try:
            self.debug_message("Creating static gump...")
            self.create_static_gump()
            self.debug_message("Creating dynamic gump...")
            self.create_dynamic_gump()
            self.debug_message("Gump created and sent")
        except Exception as e:
