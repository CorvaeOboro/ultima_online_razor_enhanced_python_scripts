"""
HOME deposit treasure - a Razor Enhanced Python Script for Ultima Online

Moves treasure maps and paragon chests from the player's backpack
...
- Optionally adjust which categories to move (treasure maps and/or paragon chests).
...
MOVE_TREASURE_MAPS = True
MOVE_PARAGON_CHESTS = True
...
TARGET_CONTAINER_SERIALS = [0x42AEFF76]
...
# Treasure Map Items - All treasure map levels
# IDs from QUEST_turn_in_items.py treasure_maps quest
TREASURE_MAP_ITEMS = [
    {"name": "Treasure Map", "id": 0x14EC, "hue": None},
    {"name": "Treasure Map", "id": 0x14ED, "hue": None},
    {"name": "Treasure Map", "id": 0x14EE, "hue": None},
    {"name": "Treasure Map", "id": 0x14EF, "hue": None},
    {"name": "Treasure Map", "id": 0x352E, "hue": None},
    {"name": "Treasure Map", "id": 0x2AAA, "hue": None},
    {"name": "Treasure Map", "id": 0x2AAB, "hue": None},
]

# Paragon Chest Items - Various paragon chest types
# IDs from QUEST_turn_in_items.py empty_paragon_chest quest
PARAGON_CHEST_ITEMS = [
    {"name": "Empty Paragon Chest", "id": 0x0E40, "hue": None},
    {"name": "Empty Paragon Chest", "id": 0x0E41, "hue": None},
    {"name": "Greater Paragon Chest", "id": 0x09AB, "hue": None},
    {"name": "a Paragon Chest", "id": 0x0E43, "hue": None},
]

def debug_message(message, color=67):
    """Send a message if DEBUG_MODE is enabled."""
    if DEBUG_MODE:
        Misc.SendMessage(f"[HomeDeposit-Treasure] {message}", color)
...
def deposit_treasure_maps(target_container):
    """Move all treasure map items from backpack into the target container."""
    total_moved = 0
    for info in TREASURE_MAP_ITEMS:
        moved = deposit_all_matching(info, target_container)
        if moved:
            total_moved += moved
            Misc.Pause(100)
    if total_moved == 0:
        debug_message("No treasure maps to deposit.", 53)
    else:
        debug_message(f"Total treasure maps deposited: {total_moved}", 65)
    return total_moved

def deposit_paragon_chests(target_container):
    """Move all paragon chest items from backpack into the target container."""
    total_moved = 0
    for info in PARAGON_CHEST_ITEMS:
        moved = deposit_all_matching(info, target_container)
        if moved:
            total_moved += moved
            Misc.Pause(100)
    if total_moved == 0:
        debug_message("No paragon chests to deposit.", 53)
    else:
        debug_message(f"Total paragon chests deposited: {total_moved}", 65)
    return total_moved

def main():
    debug_message("Starting Home Deposit: Treasure", 65)
    ...
    if MOVE_TREASURE_MAPS:
        total_items += deposit_treasure_maps(target_container)

    if MOVE_PARAGON_CHESTS:
        total_items += deposit_paragon_chests(target_container)

    if total_items > 0:
        Misc.SendMessage(f"[HomeDeposit-Treasure] Deposited {total_items} total treasure item(s)", 65)
    else:
        Misc.SendMessage("[HomeDeposit-Treasure] No treasure items found to deposit", 53)
